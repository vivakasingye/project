<?php
require 'includes/header.php';
?>
<div class="uni-banner product-single-uni-banner">
<div class="container-fluid container-large">
<div class="uni-banner-text-area">
<h1>Networking Devices</h1>
<ul>
<li><a href="index.html">Home</a></li>
<li>Networking Devices</li>
</ul>
</div>
</div>
</div>
<div class="product-details ptb-50">
<div class="container">
<div class="product-details-top-area">
<div class="row">
<div class="col-lg-6">
<div class="product-info-image-area">
<img src="assets/images/products/Telecorner Uganda limited - Router.png" alt="Telecorner Uganda limited">
</div>
</div>
<div class="col-lg-6">
<div class="product-details-info-area pl-20">
<h3>Networking Devices</h3>
<p>Our networking devices offer high-quality, reliable connectivity for your home or business. With their advanced features and user-friendly interfaces, they make it easy to connect to the internet and share your content with others. Whether you need a router, switch, or any other networking device, we have a range of models to choose from.</p>
<ul class="production-description-list">
<h4>Key Features</h4>
<div class="row-d-flex key-features">
    <div class="col-md-6">
    <li><i class="flaticon-right-arrow"></i> High-quality, reliable connectivity for your home or business</li>
    <li><i class="flaticon-right-arrow"></i> Advanced features for connecting to the internet and sharing your content with others</li>
    <li><i class="flaticon-right-arrow"></i> User-friendly interfaces for easy operation</li>
    </div>
    <div class="col-md-6">
    <li><i class="flaticon-right-arrow"></i> Available in a range of models for personal and professional use</li>
    <li><i class="flaticon-right-arrow"></i> Energy-efficient and environmentally friendly</li>
    <li><i class="flaticon-right-arrow"></i> Compact and space-saving designs</li>
    </div>
</div>
</ul>
<a class="default-button" href="contact.php"><span>Contact Us</span></a>
</div>
</div>
</div>
</div>
</div>
</div>
<div class="products pb-100 bg-f9f9f9">
<div class="container">
<div class="default-section-title pt-5">
<span>Related Products</span>
<h3>Related Products</h3>
<?php
require 'related-products.php';
?>
</div>
</div>
</div>
<?php
require 'includes/footer.php';
?>